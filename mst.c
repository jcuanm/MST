...
struct intFloatTuple find(struct node* queue, struct node* graph[], int numpoints, int name) {
  ...
  struct node* pointer = graph[name] -> next;
  float result = 2.0;
  int edgeName = 7;
...
//Javier Edits
  while(pointer != NULL) {
    ...
    if(pointer->searched == false) {
      if (graph[pointer -> name] -> searched == false){
        
        struct node* new_vertex =  (struct node*) malloc(sizeof(struct node));
        new_vertex -> name = pointer -> name;
        enqueue(queue, new_vertex);
